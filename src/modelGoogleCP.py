...
def Flexiblejobshopmodel(instance, mdl):
    ...
    task_type = collections.namedtuple(
        "task_type", "start end is_present interval"
    )
        ...
        for k in range(instance.o[j] - 1):
            for i in range(instance.g):
                for i1 in range(instance.g):
                    if (
                        instance.p[j][k + 1][i] > 0
                        and instance.p[j][k][i1] > 0
                    ):
                        mdl.Add(
                            all_tasks[j, k + 1, i].start
                            >= all_tasks[j, k, i1].end
                        )
        ...
        for i in range(instance.g - 1):
            for k in range(instance.m[i + 1]):
                for k1 in range(instance.m[i]):
                    mdl.Add(
                        all_tasks[j, i + 1, k].start >= all_tasks[j, i, k1].end
                    )
