...
def Flexiblejobshopmodel(instance, mdl):
    Z = [
        [
            [mdl.BoolVar(f"Z[{j}][{k}][{i}]") for i in range(instance.g)]
    ...
    for j in range(instance.n):
        ...
        for k in range(instance.o[j]):
            ...
            for j1 in range(instance.n):
                X[j][k][j1] = [
                    mdl.BoolVar(f"X[{j}][{k}][{j1}][{k1}]")
                    for k1 in range(instance.o[j1])
                ]

    C = [
        [
            mdl.NumVar(0, mdl.infinity(), f"C[{j}][{k}]")
            for k in range(instance.o[j])
        ]
        for j in range(instance.n)
    ]
...
def openshopmodel(instance, mdl):
    X = [
        [
            [mdl.BoolVar(f"X[{j}][{j1}][{i}]") for i in range(instance.g)]
            for j1 in range(instance.n)
        ]
        for j in range(instance.n)
    ]
    Y = [
        [
            [mdl.BoolVar(f"Y[{j}][{i1}][{i1}]") for i1 in range(instance.g)]
            for i in range(instance.g)
        ]
        for j in range(instance.n)
    ]
    ...
    for j in range(instance.n):
        for i in range(instance.g):
            for i1 in range(i + 1, instance.g):
                mdl.Add(
                    C[j][i1] - C[j][i] + M * Y[j][i][i1] >= instance.p[j][i1]
                )
    ...
    for j in range(instance.n - 1):
        for j1 in range(j + 1, instance.n):
            for i in range(instance.g):
                mdl.Add(
                    C[j1][i] - C[j][i] + M * X[j][j1][i] >= instance.p[j1][i]
                )
...
def Hybridflowshopmodel(instance, mdl):
    Y = [
        [
            [mdl.BoolVar(f"Y[{j}][{i}][{k}]") for k in range(instance.m[i])]
            for i in range(instance.g)
        ]
        for j in range(instance.n)
    ]
...
def jobshopmodel(instance, mdl):
    ...
    for j in range(instance.n):
        for i in range(1, instance.g):
            mdl.Add(
                C[j][instance.r[j][i] - 1] - C[j][instance.r[j][i - 1] - 1]
                >= instance.p[j][i]
            )
    ...
    for j in range(instance.n - 1):
        for j1 in range(j + 1, instance.n):
            for i in range(instance.g):
                mdl.Add(
                    C[j][i] - C[j1][i] - M * X[j][j1][i]
                    >= instance.p[j][instance.r[j].index(i + 1)] - M
                )
    ...
    for j in range(instance.n - 1):
        for j1 in range(j + 1, instance.n):
            for i in range(instance.g):
                mdl.Add(
                    C[j1][i] - C[j][i] + M * X[j][j1][i]
                    >= instance.p[j1][instance.r[j1].index(i + 1)]
                )
